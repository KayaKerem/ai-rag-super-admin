...
{`companyMrr = monthlyPriceTry + max(0, userCount - includedUsers) * extraUserPriceTry`}
      ...
      <Card className="border-amber-500/30 bg-amber-500/5">
        <CardContent className="py-4 text-sm text-muted-foreground">
          ...
          <div className="mt-2">
            <code className="rounded bg-muted px-1 py-0.5 text-xs">POST /internal/platform/process-downgrades</code> endpoint'i yeni fatura dönemi başladığında <code className="rounded bg-muted px-1 py-0.5 text-xs">pendingPlanId</code> olan firmaların downgrade'ini uygular. İki zorunlu header: <code className="rounded bg-muted px-1 py-0.5 text-xs">x-ai-internal-key</code> + <code className="rounded bg-muted px-1 py-0.5 text-xs">X-Edfu-Agent-Id</code> (eksikse 400). Repo'da Trigger task'i yok — Coolify'da harici cron tanımlanmalı; önerilen zamanlama: <strong>hourly</strong>.
          </div>
        </CardContent>
      </Card>
    </DocsSectionCard>
