...
import {
  ...
  mockTreasurySnapshots,
  mockTreasuryCustomers,
  mockTreasuryTransactions,
  mockTreasuryAlerts,
  mockTreasuryActions,
  mockTreasuryConfig,
  mockOpenRouterDetails,
} from './treasury-data'
...
export const handlers = [
  ...
  // ─── Treasury ─────────────────────────────────────
  http.get(`${BASE}/platform/treasury/dashboard`, async () => {
    await delay(200)
    return HttpResponse.json(mockTreasuryDashboard)
  }),

  http.get(`${BASE}/platform/treasury/snapshots`, async ({ request }) => {
    await delay(200)
    const url = new URL(request.url)
    const days = Number(url.searchParams.get('days') ?? '30')
    return HttpResponse.json(mockTreasurySnapshots.slice(0, days))
  }),

  http.get(`${BASE}/platform/treasury/customers`, async () => {
    await delay(200)
    return HttpResponse.json(mockTreasuryCustomers)
  }),

  http.get(`${BASE}/platform/treasury/transactions`, async ({ request }) => {
    await delay(200)
    const url = new URL(request.url)
    const page = Number(url.searchParams.get('page') ?? '1')
    const limit = Number(url.searchParams.get('limit') ?? '20')
    const type = url.searchParams.get('type')
    let items = mockTreasuryTransactions
  ...
  http.get(`${BASE}/platform/treasury/alerts`, async ({ request }) => {
    await delay(200)
    const url = new URL(request.url)
    const severity = url.searchParams.get('severity')
    const acknowledged = url.searchParams.get('acknowledged')
    let items = mockTreasuryAlerts
    if (severity) items = items.filter((a: any) => a.severity === severity)
    if (acknowledged === 'true') items = items.filter((a: any) => a.acknowledgedAt !== null)
    if (acknowledged === 'false') items = items.filter((a: any) => a.acknowledgedAt === null)
    return HttpResponse.json(items)
  }),

  http.patch(`${BASE}/platform/treasury/alerts/:alertId/acknowledge`, async ({ params }) => {
    await delay(200)
    const alert = mockTreasuryAlerts.find((a: any) => a.id === params.alertId)
    if (alert) {
  ...
  http.get(`${BASE}/platform/treasury/actions`, async ({ request }) => {
    await delay(200)
    const url = new URL(request.url)
    const status = url.searchParams.get('status') ?? 'pending'
    const items = status === 'all' ? mockTreasuryActions : mockTreasuryActions.filter((a: any) => a.status === status)
    return HttpResponse.json(items)
  }),

  http.post(`${BASE}/platform/treasury/actions/:actionId/approve`, async ({ params }) => {
    await delay(300)
    const action = mockTreasuryActions.find((a: any) => a.id === params.actionId)
    if (action) {
      action.status = 'approved'
      action.approvedAt = new Date().toISOString()
      action.approvedBy = 'admin-1'
    }
    return new HttpResponse(JSON.stringify(action), { status: 202 })
  }),

  http.post(`${BASE}/platform/treasury/actions/:actionId/reject`, async ({ params }) => {
    await delay(300)
    const action = mockTreasuryActions.find((a: any) => a.id === params.actionId)
    if (action) {
  ...
  http.post(`${BASE}/platform/treasury/actions/manual`, async ({ request }) => {
    await delay(300)
    const body = (await request.json()) as any
    const newAction = {
    ...
    mockTreasuryActions.push(newAction as any)
    return HttpResponse.json(newAction)
  }),

  http.get(`${BASE}/platform/treasury/config`, async () => {
    await delay(200)
    return HttpResponse.json(mockTreasuryConfig)
  }),

  http.patch(`${BASE}/platform/treasury/config`, async ({ request }) => {
    await delay(300)
    const body = (await request.json()) as any
    Object.assign(mockTreasuryConfig, body)
    return HttpResponse.json(mockTreasuryConfig)
  }),
