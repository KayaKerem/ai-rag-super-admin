...
- Endpoint tablosu (4 kolon: method | path | amaç | dönüş): `POST /platform/companies`, `GET /platform/companies`, `GET /:id`, `PATCH /:id`, `DELETE /:id`, `DELETE /:companyId/leads/:leadId/permanent`, `PATCH /:id/status`, `GET /:id/billing-events`.
- PATCH ayar alanları tablosu (4 kolon: alan | tip | default | açıklama): `name`, `customerAgentTrustLevel` (FULL_CONTROL | AUTO_MESSAGE | AUTO_ALL_QUOTE_APPROVAL | FULLY_AUTOMATIC), `autoApproveQuoteThreshold` (USD), `approvalTimeoutMinutes`, `approvalTimeoutAction` (REMIND | AUTO_SEND | HOLD). Trust-level satırında `#firma-config`'e cross-ref.
- Status lifecycle violet callout: Manuel `active | suspended | cancelled` — `trialing` ve `past_due` otomatik lifecycle'a ait, manuel atanamaz. `suspended`/`cancelled` → `SubscriptionGuard` 403 verir.
- Billing event tipleri tablosu (kod | ne zaman): 7 event — `status_change`, `plan_upgrade`, `plan_downgrade_scheduled`, `plan_downgrade_executed`, `plan_removed`, `plan_downgrade_cancelled`, `admin_override`. `plan_*` satırlarında `#fiyatlandirma-gelir` cross-ref.
- KVKK/GDPR amber callout: `DELETE /:companyId/leads/:leadId/permanent` asenkron (202 + Trigger.dev taskId).
...
- Resolve kuralı violet callout: **`resolvedTools`** sırası — (1) Firma `PricingPlan`'a bağlıysa plan `allowedTools` tek otorite; `toolPlanConfig` yok sayılır, (2) plan yoksa `toolPlanConfig.plan` → plan'ın `tools` listesi, (3) `overrides: {toolId: bool}` üstüne biner. `#fiyatlandirma-gelir` ve `#agent-tool`'a cross-ref.
- Source değerleri tablosu (3 satır): `plan` (planda var), `override` (override ile açıldı), `not_in_plan` (planda yok ama UI'da göster). `enabled` nihai hesap.
- Plan validasyon tablosu: plan ID `/^[a-z0-9-]{2,50}$/`, label zorunlu max 100 karakter, max 20 plan, `defaultPlan` mevcut olmalı, tool ID'leri `registeredTools`'da olmalı (`*` wildcard istisnası), default plan silinemez (422).
- `registeredTools` amber callout: Read-only, backend tarafından belirlenir, super admin düzenleyemez. Şu an 23 tool kayıtlı.
...
**İçerik iskeleti:**
- 1-paragraf intro: Üç metrik ailesi — Usage (maliyet), Analytics (kalite/tool/feedback), Agent Metrics (citation/human-workflow/alerts).
- Endpoint ailesi tablosu (endpoint | query | amaç): `GET /platform/companies/:id/usage` (`?months=1-12`, default 1), `/usage/current`, `/platform/usage/summary`, `/analytics`, `/agent-metrics` (`?windowDays=1-365`, default 30), `/platform/revenue`.
- Maliyet kategorileri tablosu (kategori | formül | kaynak): `ai` (OpenRouter `providerMetadata.cost` öncelikli → `computeLlmCost()` fallback), `storage` (bytes/1e9 × s3PerGbMonthUsd), `cdn` (her zaman 0 — destek kaldırıldı), `trigger` (quantity × triggerPerTaskUsd), `webSearch` ($0.007 + $0.001 × resultCount), `rerank` ($0.0025/sorgu sabit), `research`/`quotePrepare` (LLM token + bütçe rezervasyonu), `proactive_*` (economy model token).
...
- `/platform/revenue` violet callout: MRR hesabı `#fiyatlandirma-gelir`'deki formül ile aynı, burada sadece endpoint response şekli. `#fiyatlandirma-gelir`'e cross-ref.
...
- Endpoint tablosu (3 satır): `GET /platform/data-source-types`, `GET /platform/data-sources` (filtreler: `type`, `status`, `companyId`), `GET /platform/companies/:id/data-sources`.
- Status değerleri tablosu (status | anlam | renk önerisi): `active` (emerald), `syncing` (blue), `paused` (zinc), `error` (red — `errorMessage` ile).
- Response field tablosu: `id`, `companyId`/`companyName`, `type`, `name`, `config` (type-özel JSON), `status`, `errorMessage`, `itemCount`, `lastSyncAt`, `nextSyncAt`, `createdAt`.
- Crawler config amber callout: `PUT /platform/config/defaults` body.`crawlerConfig` — `cloudflareAccountId`, `cloudflareApiToken` (masked), `maxGlobalConcurrentCrawls`. Her sync → `crawl_sync` usage event. Firma limiti → `#firma-config`'in `crawlMaxPages` / `crawlMaxSources` alanlarına cross-ref.
...
| `platform-analytics` | `#fiyatlandirma-gelir` | `/platform/revenue` violet callout'ta MRR formülü için |
...
- Satır toplamı: 4 yeni section ≈ 495 satır, registry 9→13 (+10 satır), import +4 satır
